...
为什么我们需要再封装axios，因为我们在想要做这个插件的时候已经经历了几个项目，不管是否是前后台分离或者服务端是否是RESTFull类型服务，在发送请求和处理请求的时候，对于一个企业级（或者简单应用）都会存在或多或少的样板代码，那我们在实践的过程中就一步一步把这些样本代码抽离了业务，使得开发人员更容易关注于业务本身，这样就提高了开发效率，避免了一些不必要的错误，而这个模块提供了以下几点抽象：
...
由于需要服务端数据返回描述业务是否成功和业务失败的原因，因此我们来约定一下服务端返回数据的结构。
...
```js
    /**
     * 【可选】`timeout` 指定请求超时的毫秒数(0 表示无超时时间), 如果请求花费了超过 `timeout` 的时间，请求将被中断
     * <p>
     * [axios#config.timeout](https://github.com/axios/axios#request-config)
     */
    timeout = 6000
...
```js
    /**
     * 【可选】默认请求的method【'GET'| 'POST'| ’POST_JSON’| 'NATIVE' | 'ELECTRON' | ’UPLOAD’| ’PUT’|‘DELETE’】
     *  <p>
     *  提示：如果整个应用的大部分交易都需要使用**客户端代理转发请求
     *  1.涉及到前端和客户端(移动端)的交互，也就是JSBridge交互，我们已经有了一套android-viewplus 一个安卓混合客户端开发库，来解决JSBridge客户端交互流程，
     *  那么这里需要配置为'NATIVE'，这样基本上所有交易（调用$vp#ajaxMixin）都会走代理，如果某一个交易需要使用**ajax**，则在调用的时候手动设置`$vp#ajaxMixin.mode`参数进行覆盖即可
     *  2.涉及到前端和客户端(ELECTRON端)的交互，也就是JSBridge交互，通过js-bridge-electron.js 一个自定义桥接模块，来解决JSBridge客户端(ELECTRON)交互流程，
     *  那么这里需要配置为'ELECTRON'，这样基本上所有交易（调用$vp#ajaxMixin）都会走代理，如果某一个交易需要使用**ajax**，则在调用的时候手动设置`$vp#ajaxMixin.mode`参数进行覆盖即可
     */
    mode = 'POST'
```
...
```js
   ...
   * @param {String} [mode='POST'] 请求的method【'GET'| 'POST'| ’POST_JSON’| 'NATIVE' | 'ELECTRON' | ’UPLOAD’| ’PUT’|‘DELETE’】，默认使用初始化配置时候传递的`utilHttp#mode = POST`参数赋初值
   * @returns {Promise}
   */
  ajaxMixin(url, {
    params = {},
